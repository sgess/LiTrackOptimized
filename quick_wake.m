function [dE,zc,sigz] = quick_wake(z,L,Ne,Nbin,fn,pcwake)

%        [dE,zc,sigz] = quick_wake(z[,L,Ne,Nbin,fn,pcwake]);
%
...
if ~exist('pcwake','var')			% if a point-charge wake is not passed in, use file
  cmnd = ['load ' fn];
  eval(cmnd);
  idot = find(fn=='.');
  if isempty(idot)
...
dzc = zc(2)-zc(1);			% ...so particles in 1/2-end bins get...
